...
### 자동화 5: 풍부한 일일 브리핑 (1850자 한도 우회 — 2건 분할)
...
| 방법 | 권장도 | 카카오 앱 | 가족이 해야 할 일 |
|---|---|---|---|
| **A · 같은 앱에 가족을 팀원 추가** | ⭐ **권장** | 본인이 만든 앱 그대로 | 카카오 디벨로퍼스 1회 가입 + 초대 수락 |
| **B · 가족이 별도 앱 운영** | | 가족 본인 앱 별도 | 1단계 처음부터 |
...
### 방법 A · 같은 앱에 가족을 팀원으로 추가 (권장)

본인이 만든 카카오 앱에 가족을 팀원으로 끼워주는 방식. Application Credentials는 그대로 사용 가능.

#### A-1. 카카오 콘솔에서 가족 멤버 추가

1. 카카오 디벨로퍼스 → **본인 앱 → 좌측 "앱 → 멤버"** 메뉴
2. **"멤버 추가"** 버튼
3. 입력:
   - **카카오계정 (이메일)**: 가족이 카카오톡 가입 시 사용한 이메일 (예: `family@kakao.com`, `name@daum.net`, `name@nate.com`)
...
가족 본인이 카카오 디벨로퍼스에 별도 앱 생성한 경우:

1. 가족 카카오로 로그인해서 1단계 (앱 생성·플랫폼 키·동의항목) 처음부터 진행
2. HA에서 **3-1 (Application Credentials 등록)** 한 번 더 — 이름은 "가족 카카오" 같이 구분되게
3. 시크릿 창 → **3-2 (통합 추가)** → 자격 증명 선택 시 **가족 자격 증명** 선택
4. 가족 카카오로 로그인 + 동의 → `notify.kakao_noti_2` 자동 등록
